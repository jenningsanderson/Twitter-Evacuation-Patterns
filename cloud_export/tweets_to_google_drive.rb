...
wb = EpicGeo::Writers::GoogleDrive::SheetMaker.new(
		collection: "AllGeoVulnerableUsers",
		name: 		"Users-#{sheets_count}"
	)

#Make a web directory for the user (using Epic-Geo)
web_archive = HTML::ArchiveMaker.new('AllGeoVulnerableUsers')
web_archive.add_style # => default stylesheet
unless Dir.exists? 'AllGeoVulnerableUsers/kml_files'
	Dir.mkdir('AllGeoVulnerableUsers/kml_files')
end
...
users_for_kevin.each_with_index do |user_handle, index|
