...
class SandyMongoClient
	...
	def initialize(limit=nil, db_name='sandygeo', coll='geo_tweets')
		client = Mongo::MongoClient.new # defaults to localhost:27017
		db = client[db_name]
		@collection = db[coll]
		@limit = limit
	...
	def get_user_tweets(user_id_str, limit=nil)
		@limit ||= limit
		return @collection.find({"user.id_str"=>user_id_str}, {:limit=>@limit, :fields=>["user.screen_name", "text", "entities", "geo", "created_at"]})
	end
